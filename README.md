...
初めての人は、メニューの **「ソロチュートリアル」** ボタンから始められます。
**サーバー接続もP2Pも不要**で、ブラウザ内だけで完結する完全オフラインの練習場です。

- 移動 → ジャンプ → 射撃 → リロード → ヘッドショット → アイテム取得 →
  武器切替 → シールド → 回復 の **9ステップ**を、目標達成で自動的に進行
- 動かない**トレーニングダミー**が3体。倒すと数秒で復活するので何度でも練習可能
- リング（安全地帯）の収縮なし・敗北なしの安全な環境
- チュートリアル本編の仕組みは本番とまったく同じ（`GameRoom` をローカル実行）ので、
  ここで覚えた操作はそのまま対戦で通用します
- 完了後は「フリープラクティス」として自由に撃ち続けられます
